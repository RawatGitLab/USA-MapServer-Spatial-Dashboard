...
import { Users, Building, Route, MapPin, Gauge, Globe2, ChevronRight } from 'lucide-react';
...
export const KpiBar: React.FC<KpiBarProps> = ({
  ...
  const sortedByDensity = [...states].sort((a, b) => b.pop00_sqmi - a.pop00_sqmi);
  // Filter out DC for state density champion or show both
  const topDensityState = sortedByDensity.find(s => s.state_abbr !== 'DC') || sortedByDensity[0];

  return (
    <section className="bg-slate-950/60 border-b border-black dark:border-slate-800/80 px-4 lg:px-6 py-3">
      <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
        
        {/* KPI 1: Total Population */}
        <div className="bg-slate-900/80 border border-black dark:border-slate-800 rounded-xl p-3 flex flex-col justify-between hover:border-slate-700 dark:hover:border-slate-600 transition">
        ...
        <div className="bg-slate-900/80 border border-black dark:border-slate-800 rounded-xl p-3 flex flex-col justify-between hover:border-slate-700 dark:hover:border-slate-600 transition">
          ...
          <div className="mt-1.5">
            ...
            <p className="text-[10px] text-slate-400">9 Sub-regions tracked</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-black dark:border-slate-800 rounded-xl p-3 flex flex-col justify-between hover:border-slate-700 dark:hover:border-slate-600 transition">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-[11px] font-medium text-slate-400">Cities Tracked</span>
          ...
          <div className="mt-1.5">
            ...
            <p className="text-[10px] text-slate-400">Inc. 50 State Capitals</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-black dark:border-slate-800 rounded-xl p-3 flex flex-col justify-between hover:border-slate-700 dark:hover:border-slate-600 transition">
          ...
          <div className="mt-1.5">
            ...
            <p className="text-[10px] text-slate-400">Interstates & US Routes</p>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-slate-900/80 border border-black dark:border-slate-800 rounded-xl p-3 flex flex-col justify-between hover:border-blue-500/50 cursor-pointer group transition"
        >
          <div className="flex items-center justify-between text-slate-400">
